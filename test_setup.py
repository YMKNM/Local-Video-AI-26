...
def test_resource_monitor():
    ...
    print(f"  [OK] GPU: {status.gpu.name}")
...
def test_directml():
    ...
    try:
        ...
        if 'DmlExecutionProvider' in providers:
            print(f"  [OK] DirectML is available!")
        else:
            print(f"  [WARN] DirectML not available (CPU fallback will be used)")
            
    except ImportError:
        print(f"  [WARN] ONNX Runtime not installed")
...
def test_ffmpeg():
    ...
    print(f"  [OK] FFmpeg found: {ffmpeg.ffmpeg_path}")
    ...
    print(f"     H.264: {'yes' if h264 else 'no'}")
    print(f"     H.265: {'yes' if h265 else 'no'}")


def test_frame_writer():
    """Test frame writing"""
    print("\n[TEST] Testing Frame Writer...")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert path.exists(), "Frame file was not created"
        print(f"  [OK] Frame written: {path.name}")


def test_video_assembly():
    """Test video assembly (requires FFmpeg)"""
    print("\n[TEST] Testing Video Assembly...")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert success and output_path.exists(), "Video was not created"
        size_kb = output_path.stat().st_size / 1024
        print(f"  [OK] Video created: {size_kb:.1f} KB")


def test_generation_planner():
    """Test generation planning"""
    print("\n[TEST] Testing Generation Planner...")
